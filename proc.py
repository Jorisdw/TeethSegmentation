...
class procrustes:
    ...
    #     mtx2 = array(data2.copy(),'d')
        ...
        mtx1, meanVector1 = self.center(mtx1)
        mtx2, meanVector2 = self.center(mtx2)

        if ((not any(mtx1)) or (not any(mtx2))):
            raise ValueError("input matrices must contain >1 unique points")

        mtx1, norm1 = self.normalize(mtx1)
        mtx2, norm2 = self.normalize(mtx2)


        # transform mtx2 to minimize disparity (sum( (mtx1[i,j] - mtx2[i,j])^2) )
        mtx2 = self.match_points(mtx1, mtx2)
    ...
    the matrix, and the vectors have nonzero components in that dimension")
        ...
        disparity = self.get_disparity(mtx1, mtx2)

        return mtx1, mtx2, disparity, norm1, norm2, meanVector1, meanVector2
    ...
    def match_points(self,mtx1, mtx2):
    ...
    def get_disparity(self,mtx1, mtx2):
